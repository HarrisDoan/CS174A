...
export class Assignment3 extends Scene {
  ...
  draw_planets(context, program_state, model_transform, t) {
    ...
    let theta = t; //Orbit speed as variable t

    //List of all matrix transformations on all planets:

    let Rt1 = Mat4.rotation(theta, 0, 1, 0); //Planet 1 rotation
    let Tr1 = Mat4.translation(5, 0, 0); //Planet 1 translation - 5 units
    let Rt2 = Mat4.rotation(theta / 2, 0, 1, 0); //Planet 2 rotation - half orbit of planet 1
    let Tr2 = Mat4.translation(8, 0, 0); //Planet 2 translation - 5+3 = 8 units
    let Rt3 = Mat4.rotation(theta / 3, 0, 1, 0); //Planet 3 rotation - third orbit of planet 1
    let Tr3 = Mat4.translation(11, 0, 0); //Planet 3 translation - 8+3 = 11 units
    let Rt4 = Mat4.rotation(Math.sin(t), 1, 0, 0); //Planet 3 - wobble using sin(t)
    let ScRing = Mat4.scale(3.5, 3.5, 0.1); //Scale the Planet's ring
    let Rt5 = Mat4.rotation(theta / 4, 0, 1, 0); //Planet 4 rotation - fourth orbit of planet 1
    let Tr5 = Mat4.translation(14, 0, 0); //Planet 4 translation - 11+3 = 14 units
    let Rt6 = Mat4.rotation(t, 0, 1, 0); //Moon rotation same as orbit = t
    let Tr6 = Mat4.translation(-2, 0, 0); //Moon translation

    //Planet 1-4, Moon, Ring

    var planet1_transform = model_transform;
    planet1_transform = planet1_transform.times(Rt1).times(Tr1);
    ...
    planet2_transform = planet2_transform.times(Rt2).times(Tr2);
    if (t % 2 == 0) {
    ...
    planet3_transform = planet3_transform.times(Rt3).times(Tr3).times(Rt4);
    ...
    var ring_transform = planet3_transform.times(ScRing);
    ...
    planet4_transform = planet4_transform.times(Rt5).times(Tr5);
    ...
    moon_transform = moon_transform.times(Rt6).times(Tr6);
    ...
    let InvTr = Mat4.translation(0, 0, 5);

    this.planet_1 = Mat4.inverse(planet1_transform.times(InvTr));
    this.planet_2 = Mat4.inverse(planet2_transform.times(InvTr));
    this.planet_3 = Mat4.inverse(planet3_transform.times(InvTr));
    this.planet_4 = Mat4.inverse(planet4_transform.times(InvTr));
    this.moon = Mat4.inverse(moon_transform.times(InvTr));
  }
  ...
  display(context, program_state) {
    ...
    var radius = 2 + Math.sin(((2 * Math.PI) / 10) * t);
...
class Ring_Shader extends Shader {
  ...
  fragment_glsl_code() {
    ...
    return (
      ...
      `
        void main(){
            float scalar = cos(18.0 * distance(point_position.xyz, center.xyz));
